...
function checkLogin($email, $password)
{
	...
	filter_var($email, FILTER_VALIDATE_EMAIL) or die("Email không hợp lệ"); // kiểm tra email hợp lệ

	$query = "SELECT `password` FROM `nguoi_dung` WHERE `email`= '$email'";
	$result = mysqli_query($conn, $query);
	$getpass = mysqli_fetch_assoc($result)["password"] or exit("Tài khoản chưa được đăng kí!");
